...
class HungarianMatcher:
    ...
    def __init__(self, cost_bce: float = 1.0, cost_dice: float = 1.0, cost_size: int = 128) -> None:
        self.cost_bce = float(cost_bce)
        self.cost_dice = float(cost_dice)
        self.cost_size = int(cost_size)

    @torch.no_grad()
    def __call__(self, pred_masks: torch.Tensor, target_masks: torch.Tensor) -> MatchResult:
