...
class MaskDecoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # decoder 第一版保持很轻量：把 query 和 patch token 投到同一空间，
    ...
    def forward(
        ...
        # logits[b, q, n] 表示第 q 个实例 query 与第 n 个图像 patch 的相似度。
        ...
        if image_size is not None and image_size != patch_grid:
            # patch 级 mask 上采样回图像分辨率，后续再与 SAM 伪标签计算损失。
